...
const generator: GroupGenerator = {
  ...
  generate: async (context: GenerationContext): Promise<GroupWithData[]> => {
    ...
    // we query Hive API with eth leaderboard data and keep only those with a Hive rank < 10000
    const ensAccountsExistingonHive = await hiveProvider.getInfluencersAboveMaxRank(ethLeaderboardInfluencers, 10000, ["Ethereum", "Crypto"]);
