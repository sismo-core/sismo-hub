...
const generator: GroupGenerator = {
  ...
  generate: async (context: GenerationContext): Promise<GroupWithData[]> => {
    ...
    const hiveInfluencersEthereum = hiveProvider.getInfluencersFromClusterWithMinimumFollowers("Ethereum", 0, 10000)
    ...
    const data : FetchedData = await ensProvider.getAllAddresses(ethInfluencerFromHive)

    return [
      {
        name: "ens-influencers",
