...
describe("Test cli command", () => {
  it("should have default to local stores", async () => {
    ...
    expect(testProgram.opts<GlobalOptions>().availableGroupStore).toBeInstanceOf(LocalFileStore);
    expect(testProgram.opts<GlobalOptions>().groupStore).toBeInstanceOf(LocalGroupStore);
    expect(testProgram.opts<GlobalOptions>().groupGeneratorStore).toBeInstanceOf(
      LocalGroupGeneratorStore
    );
  });
  ...
  it("should have memory group store", async () => {
    const testProgram: Command = createEmptyCommand();
    await testProgram.parseAsync(["node", "./data-sources", "--storage-type", "memory"]);
    expect(testProgram.opts<GlobalOptions>().availableDataStore).toBeInstanceOf(
      MemoryAvailableDataStore
    );
    expect(testProgram.opts<GlobalOptions>().availableGroupStore).toBeInstanceOf(MemoryFileStore);
    expect(testProgram.opts<GlobalOptions>().groupStore).toBeInstanceOf(MemoryGroupStore);
    expect(testProgram.opts<GlobalOptions>().groupGeneratorStore).toBeInstanceOf(
      MemoryGroupGeneratorStore
    );
  });
  ...
  it("should have aws group store", async () => {
    const testProgram: Command = createEmptyCommand();
    await testProgram.parseAsync(["node", "./data-sources", "--storage-type", "aws"]);
    expect(testProgram.opts<GlobalOptions>().availableDataStore).toBeInstanceOf(
      DynamoDBAvailableDataStore
    );
    expect(testProgram.opts<GlobalOptions>().availableGroupStore).toBeInstanceOf(S3FileStore);
    expect(testProgram.opts<GlobalOptions>().groupStore).toBeInstanceOf(DynamoDBGroupStore);
    expect(testProgram.opts<GlobalOptions>().groupGeneratorStore).toBeInstanceOf(
      DynamoDBGroupGeneratorStore
    );
  });
  ...
  it("should have stdout logger service", async () => {
    const testProgram: Command = createEmptyCommand();
    await testProgram.parseAsync(["node", "./data-sources", "--logger-type", "stdout"]);
    expect(testProgram.opts<GlobalOptions>().logger).toBeInstanceOf(StdoutLogger);
  });

  it("should have memory logger service", async () => {
    const testProgram: Command = createEmptyCommand();
    await testProgram.parseAsync(["node", "./data-sources", "--logger-type", "memory"]);
    expect(testProgram.opts<GlobalOptions>().logger).toBeInstanceOf(MemoryLogger);
  });

  it("should have local file logger service", async () => {
    const testProgram: Command = createEmptyCommand();
    await testProgram.parseAsync(["node", "./data-sources", "--logger-type", "local-file"]);
    expect(testProgram.opts<GlobalOptions>().logger).toBeInstanceOf(LocalFileLogger);
  });
});
